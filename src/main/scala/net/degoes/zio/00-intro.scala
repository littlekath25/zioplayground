...
//    self =>
//    def map[B](f: A => B): ZIO[R, E, B] = ZIO[R, E, B](r => self.run(r).map(f))
//
...
//
//  val readInt =
//    Console.readLine.flatMap(string => ZIO.attempt(string.toInt)).orDie
//
//  /** EXERCISE
//    *
...
//        onFailure(new java.io.IOException("Could not load body!"))
